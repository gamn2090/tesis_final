...
function ingresar_solicitud_rein($cedula,$proceso,$razon,$fecha,$conn)
{
...
{   if($proceso=="Retiro")
	{		
		if($nivel==$bandera)
		{
			$query="SELECT * FROM solicitudes_ret WHERE (proceso LIKE '$proceso%') ORDER BY exp";
			$result=$conn->Execute($query);	
		}
		else
		{
...
function cargar_datos_estudiante($proceso,$numero_soli,$cedula,$nivel,$exp,$conn)
{	
	if($proceso=='Retiro')
	{
	...
	if($algo==1)
	{
		...
		if($exp!='-1')
		{
			mostrar_datos_estudiante_coord($grupo,$aval,$cedula,$nombre,$apellido,$razon,$promedio,$discapacidad,$nacionalidad,$solicitudes,$proceso,$cant_soli,$fecha);

		}
		else
		{
			mostrar_datos_estudiante_sec($grupo,$numero_soli,$cedula,$nombre,$apellido,$razon,$promedio,$discapacidad,$nacionalidad,$solicitudes,$proceso,$cant_soli,$fecha);
		}
	}
	else
	{
...
?>	
	<form class="cbp-mc-form" name="Evaluar_estudiante" id="Evaluar_estudiante" action="seleccion_opciones.php" method="POST">
        ...
        <?php if($solicitud=='Retiro' || $solicitud=='Reingreso'){ ?>
...
{
	$query="SELECT COUNT(*) cont FROM notas WHERE ((per = '1' AND nota > '4') OR (per = '2' AND nota > '4')) AND cedula = '".$cedula."' ";
	$result = $conn->Execute($query);
	if($result==false)
	{
		echo "error al recuperar: ".$conn->ErrorMsg()."<br>" ;
...
function decision_rein($fecha,$grupo,$cedula,$razon,$nombre,$apellido,$discapacidad,$promedio,$solicitudes,$solicitud_actual,$aval,$cant_soli,$periodo,$conn  )
{
	$query="SELECT * FROM razon_proceso WHERE (proceso LIKE '%$solicitud_actual%') AND (razon LIKE '%$razon%')";
	$result=$conn->Execute($query);
	if($result==false)
	...
	//============================================tiempo de solicitud=============================================================
		...
		$query="SELECT * FROM consideraciones WHERE proceso LIKE '%$solicitud_actual%' AND consideracion LIKE '%Repite%'";	
		$result2=$conn->Execute($query);
		if($result==false)
		{
			echo "error al recuperar: ".$conn->ErrorMsg()."<br>" ;
		...
		else
		{
			while(!$result2->EOF)
			{
				for($i=0, $max=$result2->FieldCount(); $i<$max; $i++)
				{
					$porcentaje_medidas=$result2->fields['ponderacion']/100;
				}
				$result->MoveNext();
			}
		}		
		...
		//=====================================================Prosecucion académica================================================
		$query="SELECT * FROM consideraciones WHERE proceso LIKE '%$solicitud_actual%' AND consideracion LIKE '%Prosec%'";	
		$result2=$conn->Execute($query);
		if($result==false)
		{
			echo "error al recuperar: ".$conn->ErrorMsg()."<br>" ;
		...
		else
		{
			while(!$result2->EOF)
			{
				for($i=0, $max=$result2->FieldCount(); $i<$max; $i++)
				{
					$porcentaje_prosecucion=$result2->fields['ponderacion']/100;
				}
				$result->MoveNext();
			}
		}		
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				if($result==false)
				{
					echo "error al recuperar:asdasdsa ".$conn->ErrorMsg()."<br>" ;
				}
				else
				{
					while(!$result->EOF) 
				...
				{
					$aprobado_temp=$aprobado_temp+($puntaje*$porcentaje_grupo);
					$punt_grupo="Pertenece al grupo 7 con ".$cont_sem." semestres con mas del 50% de creditos aprobados";
				}
				else
				{
					$reprobado_temp=$reprobado_temp+($puntaje*$porcentaje_grupo);
